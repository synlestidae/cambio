...
impl TryFromRow for Order {
    fn try_from_row<'a>(row: &Row<'a>) -> Result<Self, TryFromRowError>
        ...
        let owner_id = try!(owner_id_match.ok_or(TryFromRowError::missing_field("Order", "owner_id")));
        ...
        if (buy_asset_units_i64 < 0) {
            return Err(TryFromRowError::bad_value("Order", "buy_asset_units", buy_asset_units_i64));
        }
        ...
        match (Denom::parse(&sell_asset_denom_string), Denom::parse(&buy_asset_denom_string)) {
            ...
            _ => return Err(TryFromRowError::new("Could not parse the buy_asset_denom or sell_asset_denom field"))
        }
