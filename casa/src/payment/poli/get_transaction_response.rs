...
mod test {
    ...
    fn test_get_transaction_response_deserializes() {
        ...
        assert_eq!(
            tx_response.transaction_ref_no.unwrap().to_string(),
            "996108109898"
        );
        ...
        assert_eq!(
            tx_response.established_date_time.unwrap().to_string(),
            "2018-02-27 15:19:55.063"
        );
        assert_eq!(
            tx_response.start_date_time.unwrap().to_string(),
            "2018-02-27 15:19:55.063"
        );
        assert_eq!(
            tx_response
                .merchant_established_date_time
                .unwrap()
                .to_string(),
            "2018-02-27 15:19:54.123"
        );
        assert_eq!(tx_response.merchant_reference.unwrap().0, "MyRef01");
        assert_eq!(
            tx_response.transaction_status_code,
            Some(TransactionStatusCode::EulaAccepted)
        );
        ...
        assert_eq!(
            tx_response.merchant_defined_data.unwrap().0,
            "MyDefinedData"
        );
