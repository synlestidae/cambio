...
impl TryFromRow for ContactInfo {
    fn try_from_row<'a>(row: &Row<'a>) -> Result<Self, TryFromRowError> {
        ...
        let main_intl_phone_number =
            try!(get_value_option("Account", "main_intl_phone_number", row));
        let secondary_intl_phone_number = try!(get_value_option(
            "Account",
            "secondary_intl_phone_number",
            row,
        ));
